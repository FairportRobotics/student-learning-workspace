# Topics #


## Source Control ##
- RCS
- Conflicts
- Merging
- Branches
- CI/CD
- [git reference](https://github.com/k88hudson/git-flight-rules)
	- status
	- init
	- branching
	- index
	- merge
	- push
	- pull
	- ignore
	- reset

## PID Systems ##
- [Understanding PID Control, Part 1: What is PID Control?](https://www.youtube.com/watch?v=wkfEZmsQqiA)
- [Understanding PID Control, Part 2: Expanding Beyond a Simple Integral](https://www.youtube.com/watch?v=NVLXCwc8HzM)
- [Understanding PID Control, Part 3: Expanding Beyond a Simple Derivative](https://www.youtube.com/watch?v=7dUVdrs1e18)
- [Understanding PID Control, Part 4: A PID Tuning Guide](https://www.youtube.com/watch?v=sFOEsA0Irjs)
- [Tuning PID](https://github.com/MTHSRoboticsClub/Documentation/blob/master/talon-PIDF-proceedures.md)
- [Open vs Closed Loop Systems](https://www.youtube.com/watch?list=PLFFbyVlrhQnvaEyHXkfxpKRVzhDocc6eT&v=O-OqgFE9SD4)

## Robot Review ##

- Identify Parts of the robot
	- Rio
	- Drivetrain
	- PDP
	- Talons
	- Motors
	- CanBus
	- Router
- Actuators
- Limit Switches
- Different Drivetrain Types
- Pneumatics
- NavX and Pigeon Gyro
- PID
- Motion Profiling
- Encoders
- CTRE Talon SRX
- OI
	- Joystick
	- Gamepad
- Driver's Station
	- Shuffleboard
	- Console
	- Game Phases
	- USB
	- Event Graphs
 
## Robocode ##
- [Main Page](https://robocode.sourceforge.io/)

## Concurrency ##
- Thread
- [Coding a thread](https://dzone.com/articles/java-thread-tutorial-creating-threads-and-multithr)
- shared memory space
- synchronization
- [locks](https://winterbe.com/posts/2015/04/30/java8-concurrency-tutorial-synchronized-locks-examples/)
	- mutex
	- reentrant
	- semaphores
- Blocking is slow
- [CAS](http://tutorials.jenkov.com/java-concurrency/compare-and-swap.html)
- Functional safety
- Atomicity
- [CAP Theorem and Consistency Models](https://aphyr.com/posts/313-strong-consistency-models)

## Java ##
- [Programming Done Right](https://frc-pdr.readthedocs.io/en/latest/)
- [Reference](https://github.com/MTHSRoboticsClub/Documentation/blob/master/Java-Programming-team-3528.pdf)
- packages
- classes
- variables
- variable types
- arrays
- hashmaps
- libraries
- for loops
- if statements
- while loops
- functions
- scope
- JVM
- GC
- Bytecode
- Jar files
- gradle
- IDEs
- debugging
- logging
- IO

## Robot Programming ##
- [Reference](https://github.com/MTHSRoboticsClub/Documentation/blob/master/IntroProgrammingFRCRobots2012.pdf)
- Command Based
- RoboRIO
- Networking
- Shuffleboard
- Libraries
- Pathfind
- PID
- OI
- Talons
- Gyro


## Software ##
- Classes
- Inheritance
- Polymorphism
- Overloading
- Signatures
- Design
- Design Patterns
- Interfaces
- Functional Programming
- compiled vs script
- LISP like
- Pure functional
- Assembly

## Building a Computer ##
-  [PC Part Picker](https://pcpartpicker.com/)
-  [ECC Memory](https://en.wikipedia.org/wiki/ECC_memory)
-  [Amazing Deal on an Intel Nic!](https://www.ebay.com/itm/NEW-Intel-OEM-I350T4V2BLK-Ethernet-Server-Adapter-Gigabi-4-Port-RJ45-PCI-Express/112400469306?hash=item1a2b970d3a:g:NHEAAOSwhvFZEIBT:sc:USPSFirstClass!14450!US!-1:rk:2:pf:0)
	-  [oops](https://forums.servethehome.com/index.php?threads/comparison-intel-i350-t4-genuine-vs-fake.6917/)
	-  [well...maybe not so bad](https://forum.netgate.com/topic/67125/fyi-those-cheap-chinese-i350-t4-am4-nics-on-ebay-work)
-  Motherboards
-  Sata
-  Bios
-  Expandability
-  Heat Management
-  Fans
-  [Size and purpose](https://old.reddit.com/r/PFSENSE/comments/8l0v8z/help_with_purchasing_new_system/)
-  Return Policy, Rebates, Warantees
-  GPUs
-  CPU / Cores
-  Slots
- [Lets Review](https://www.parkytowers.me.uk/thin/hp/t620/index.shtml)



## Networking ##
- [Comprehensive Guide to pfSense 2.3 Part 4: Networking Crash Course](https://www.youtube.com/watch?v=mgEMGoFIots)
- OSI Model
- [Socket Programming](https://gist.github.com/tacksoo/4733928)
- Ports
- UDP/TCP
- Routing
- Firewalls
- Hub vs Switch
- DNS
- VPNs
- HTTP / HTTPS
- [Ha! We just talked about this!](https://www.itnews.com.au/news/china-systematically-hijacks-internet-traffic-researchers-514537)


## Operating Systems ##
- Real time vs scheduled
- Kernel space
- Processes
- CPU management
- Memory management
- Security
- IO
- Monitoring
- Booting
- Logs
- Hardware Abstraction
- Virtualization

## Linux ##
- [Basic commands](https://maker.pro/linux/tutorial/basic-linux-commands-for-beginners)
- File System
- identity
- home
- shell environment
- system information
- commands
	- man
	- rsync
	- vi
	- od
	- sort
	- uniq
	- grep
	- sed
	- awk
	- more / less
	- df
	- du
- pipe
- stdin/stdout/stderr
- ctrl
- signals

## Game Strategy ##

## Security ##
- [Basic Cryptography](https://www.youtube.com/watch?v=jhXCTbFnK8o)
- [Diffie Hellman (public/private)](https://www.youtube.com/watch?v=YEBfamv-_do)
	- [Funny Example](https://np.reddit.com/r/math/comments/3tn1xq/what_intuitively_obvious_mathematical_statements/cx7np4t/?context=3)
- gpg /pgp
- Hashing
- Linux privileges
- DDOS
- DNS Spoofing
- Passwords
- Viruses
- Trojans
- The Human Element
- LIDS / Snort
- Identity
- SCP / SSH vs FTP
- Honeypots
- Program Exploits
